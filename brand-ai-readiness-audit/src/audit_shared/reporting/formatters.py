...
class MarkdownFormatter:
    ...
    def generate(cls, report: FinalReport) -> str:
        ...
        lines.append(f"**Pages evaluated:** {report.evaluation_scope.pages_evaluated}")
        lines.append(f"**Crawl status:** {cls._escape(report.evaluation_scope.status)}")
        lines.append("")
        ...
        lines.append(f"- Findings excluded during evidence validation: {report.diagnostics.excluded_findings}")
        lines.append("")
...
class TerminalFormatter:
    ...
    def generate(cls, report: FinalReport) -> str:
        ...
        lines.append(f"Score: {report.summary.score}/100")
        lines.append(f"Pages evaluated: {report.evaluation_scope.pages_evaluated}")
        lines.append(f"Crawl status: {report.evaluation_scope.status}")
        lines.append("")
        ...
        if report.diagnostics.stage_timing:
            lines.append("Stage Timing")
            lines.append("--------------------------------")
            lines.append(f"Crawling                 {report.diagnostics.stage_timing.crawling:.2f}s")
            lines.append(f"Rule engines             {report.diagnostics.stage_timing.rule_engines:.2f}s")
            lines.append(f"Grouping                 {report.diagnostics.stage_timing.grouping:.2f}s")
            lines.append(f"Evidence validation      {report.diagnostics.stage_timing.evidence_validation:.2f}s")
            lines.append(f"NLP                      {report.diagnostics.stage_timing.nlp:.2f}s")
            lines.append(f"GenAI                    {report.diagnostics.stage_timing.genai:.2f}s")
            lines.append(f"Reporting                {report.diagnostics.stage_timing.reporting:.2f}s")
            lines.append("--------------------------------")
            lines.append(f"Total                    {report.diagnostics.stage_timing.total:.2f}s")
            lines.append("")
            
        if report.diagnostics.genai:
            lines.append("GenAI Diagnostics")
            lines.append("--------------------------------")
            lines.append(f"Eligible groups:          {report.diagnostics.genai.eligible_groups}")
            lines.append(f"Requests attempted:       {report.diagnostics.genai.requests_attempted}")
            lines.append(f"Successful:               {report.diagnostics.genai.successful}")
            lines.append(f"Rate limited (429):       {report.diagnostics.genai.rate_limited}")
            lines.append(f"Timeouts:                 {report.diagnostics.genai.timeouts}")
            lines.append(f"Provider failures:        {report.diagnostics.genai.provider_failures}")
            lines.append(f"Invalid responses:        {report.diagnostics.genai.invalid_responses}")
            lines.append(f"Skipped by budget:        {report.diagnostics.genai.skipped_by_budget}")
            lines.append(f"Total GenAI time:         {report.diagnostics.genai.total_duration_seconds:.2f}s")
            lines.append("")
